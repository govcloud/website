...
* D3.js for our Data Visualizations which you will in a moment hear about by the amazing Laurent Goderre
  * Seriously this guy knows his stuff and has taught me so much
  * We met through the wet-boew GitHub repository and he has taught me so much
  * Incidentally without his help wet-boew integration with platforms such as Drupal would have been much harder

Close to my heart though is we have a stellar Web Content Management Systems team and maintainers that have been using `Drupal` for the past 5+ years and all of whom develop (for the most part) in the open.

Next we have a search team that continues to use `Apache Solr` the open source enterpise search platform (built on Apache Lucene) for many of our search pages and product data pages.

Though I should mention the search team and some other ones are also looking at ElasticSearch mostly from an `ELK` stack perspective.

* ElasticSearch
* Logstash
* Kibana (D3.js)

What is perhaps the most exciting is the amount of teams we as the Cloud Operations team assist with moving toward microservices by first containerizing their applications.

We have various teams doing this so rather then mention acronyms I will just mention the programming language which is perhaps the most relevant.

* C#, .NET (windows)
* JS / Node.js
* Java (set your memory limits)
* Python
* PHP
* SAS
* Stata

Finally as an organization along with every cloud vendor are taking quite a thorough look at what a pretty awesome person says is "the operating system of the cloud".
...
* Ability to shape the product and align to your own departments vision
...
* 5+ years showing the value of what proper content architecture can do
...
Providing support to countless other departments but now assisting NRCAN with their FRP Initiative.
...
* Work with the amazing people at CDS (seriously know joke those are some of the most knowledgeable people you could ever meet) hosting the monthly GoC Cloud Native Meetups
...
* Attract developer talent
  ...
  For myself, thanks to some of the Drupal's team open source work, we managed to snag a brilliant developer straight out of Waterloo (screw you private companies) who is personally the most knowledgeable developer I have ever worked with and has become my best friend.

  These type of talent drive new energy and innovation into your department.
...
Some people worry that an open source system is easier to hack; the reality, though, is the team of users and dedicated security experts are the best defense you’ll find.
...
So I am only going to talk about Drupal at a high level as there is a full demonstration at 3:30 being given by Robin Galipeau showing Drupal WxT + Solr in action.
...
> **Drupal 8** provides the following features built-in for authoring (to only name a few):
...
> **Drupal 8** is the most accessible `CMS/CMF` without exception. Notable improvements in core:
...
- Every single core theme is responsive
- Administration toolbar was build mobile first
- Drupal Core provides convenient modules to assist:
  - Breakpoint (toolbar)
  - Responsive Image
- Integration with: Bootstrap / WxT (WET-BOEW) Framework

# Multilingual

- Language Handling:
  - Natively install in 94 languages
  - Assign language to every context in system
  - Admin interface language selection option per user

- Interface Translation
  - Automated downloads & updates
  - Protected local translations
  - Plural version editable on integrated interface
...
- Configuration
  - Everything in system is translatable
  - Built-in responsive / translation interface
  - Overview screen for translators, contextual translation tabs for site builders

# Tour

- Context-sensitive guided tour of the site interface via tooltips
- Draw user's attention to critical components of interface
- Guide a user through a workflow
- Multilingual / Exportable
- Easy to create Tours via API or interface

# Content Workflow

- All content entities:
  - Built-in revision support
  - Improved `moderation state` transition handling
  - Rich UI configuration of base / custom fields
- Views is now part of Core
- CKEditor is wholly integrated by default
- Quick Edit

# Content Staging

Thanks to the `Deploy` suite of modules:

`Deploy`, `Multiversion`, `Replication`, `Workspace`, `RELAXed Web Services`, and `Trash`

- Cross-site content staging
- Content branching (ala `git`)
- Preview your site
- Offline browse / publish
- Content recovery / Audit logs

For more info: http://www.drupaldeploy.org
...
- Configuration API provides central place to store configuration data
- The Data consists of:
  - Simple Configuration: Sitewide settings
  - Complex Configuration: `Content Types` / `Views` / `Entity Reference`
- All configuration data representing the whole state of the site can be easily exported.

# Configuration Schema

- Describes the structure of configuration files.
  - Typecasts configuration for type consistency
  - Automated persistence of configuration entity
  - Automated generation of the configuration translation user interface
- Easily sync between configuration from yaml to database
...
* Web Services directly in Drupal Core
...
* `RESTful Web Services`: Exposes entities in different defined endpoints
* `Serialization`: allows to serialize entities in formats like `HAL+JSON`, `XML`, `JSON`

# Templating

> Twig: syntax originates from Jinja and Django templates
...
We had to learn and build alot in D7 to lesson our burden and increase the community collaboration
...
carelessly but 1 year in I can say it has been an incredible time saver,
...
Drupal has added ReactJS to core which nicely aligns us with some of the WET5 work being done.

We are actively working on ensuring when WET 5 launches we also provide full support.
...
Finally this brings us to our Open Data sub-profile of WxT. Importantly this profile must run in a PostgreSQL backed environment so all modules have been extensively tested against that. It features a variety of improvements whether across the data modelling all the way through to the template layer.
...
* Solr Integrations
  * Faceted Search
  * Rendering data from multiple backend systems such as Oracle
* WCMS
  ...
  * Content Staging / Deployment
    ...
    * Decouple the presentation layer and abstract it as is easier to change
  * Blog
  * Chat with an Expert
...
* Plugin Architecture
  * Azure Plugins
  * Artifactory
* Building and Deploying to multiple Kubernetes Clusters
...
Hopefully I won't add to the tribal warfare so will try to get everyone to agree on a couple of facts as both technologies have awesome use-cases and valid pain points.
